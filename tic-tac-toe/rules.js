...
var moveCell;
function begin_play(){
	p1id = document.getElementById("player1_id");
	p2id = document.getElementById("player2_id");
	turnInfo = document.getElementById("turn_info");
	if(isEmpty(p1id.value)||isEmpty(p2id.value)){
...
function play() {
	...
	if(!found){
		alert("Invalid move. Try entering a value from the table.");
		return;
	}
	if(board_state[index] != -1){
		alert("Please select a box that has not been played yet.");
		return;
	}
